...
export default async function dashboardMentees() {
  ...
  const currentUserEmail = await currentUser.email;

  const topThreeMentorsList = await getTopThreeMentors(currentUserEmail);

  // doesnt work yet
  const mentorUserData = await getUserById();
  ...
  return (
    <main>
      ...
      {await topThreeMentorsList?.map((d) => {
        return <p key={`dataID-select-${d.mentorUserId}`}>{d.mentorUserId}</p>;
      })}
